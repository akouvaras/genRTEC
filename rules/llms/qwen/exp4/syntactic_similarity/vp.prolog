...
initiatedAt(status(M)=proposed, T) :-
    ...
    not holdsAt(status(M)=proposed, T),
    not holdsAt(status(M)=voting, T),
    not holdsAt(status(M)=voted, T).
...
initiatedAt(status(M)=null, T) :-
    happensAt(declare(Ag, M, _Result), T),
    role_of(Ag, chair),
...
initiatedAt(voted(Ag, M)=null, T) :-
    ...
    holdsAt(voted(Ag, M)=aye, T).

initiatedAt(voted(Ag, M)=null, T) :-
    happensAt(start(status(M)=null), T),
    holdsAt(voted(Ag, M)=nay, T).
