...
initiatedAt(status(M)=null, T) :-
    happensAt(declare_outcome(Chair, M), T),
    role_of(Chair, chair),
...
initiatedAt(voted(V,M)=aye, T) :-
    happensAt(cast_vote(V,M, aye), T),
    holdsAt(status(M)=voting, T).

initiatedAt(voted(V,M)=nay, T) :-
    happensAt(cast_vote(V,M, nay), T),
    holdsAt(status(M)=voting, T).
...
initiatedAt(outcome(M)=carried, T) :-
    happensAt(declare_outcome(Chair, M, carried), T),
    role_of(Chair, chair),
    holdsAt(status(M)=voted, T).

initiatedAt(outcome(M)=not_carried, T) :-
    happensAt(declare_outcome(Chair, M, not_carried), T),
    role_of(Chair, chair),
    holdsAt(status(M)=voted, T).
