...
holdsFor(pow(accept_quote(Cons,Merch,GD))=true, I) :-
    ...
    holdsFor(suspended(Merch, merchant)=true, IsMerch),
    holdsFor(suspended(Cons,  consumer)=true, IsCons),
    relative_complement_all(Iq, [IsMerch, IsCons], I).
...
initiatedAt(obl(send_EPO(Cons,Merch,iServer,GD))=true, T) :-
	happensAt(start(contract(_Merch,Cons,GD)=true), T).

initiatedAt(obl(send_goods(Merch,Cons,iServer,GD))=true, T) :-
	happensAt(start(contract(Merch,_Cons,GD)=true), T).

terminatedAt(obl(send_EPO(Cons,Merch,iServer,GD))=true, T) :-
	happensAt(send_EPO(Cons,iServer,GD,Pr), T),
	price(GD,Pr).

terminatedAt(obl(send_goods(Merch,Cons,iServer,GD))=true, T) :-
...
terminatedAt(obl(send_goods(Merch,Cons,iServer,GD))=true, T1, T, T2) :-
