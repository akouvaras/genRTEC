...
initiatedAt(quote(Cons, Merch, GD)=true, T) :-
    happensAt(present_quote(Merch, Cons, GD, _Pr), T).

terminatedAt(quote(Cons, Merch, GD)=true, T) :-
    happensAt(accept_quote(Cons, Merch, GD), T).

%--------------- contract -----------%

initiatedAt(contract(Cons, Merch, GD)=true, T) :-
    happensAt(accept_quote(Cons, Merch, GD), T),
    holdsAt(quote(Cons, Merch, GD)=true, T),
...
holdsFor(pow(Cons, accept_quote, Merch, GD)=true, I) :-
    holdsFor(quote(Cons, Merch, GD)=true, Iq),
...
initiatedAt(obl(Cons, pay, Merch, GD)=true, T) :-
    happensAt(start(contract(Cons, Merch, GD)=true), T).
...
terminatedAt(obl(Cons, pay, Merch, GD)=true, T) :-
    happensAt(end(contract(Cons, Merch, GD)=true), T).
